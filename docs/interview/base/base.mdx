...
XSS（跨站脚本攻击）是指攻击者在返回的 HTML 中嵌入 javascript 脚本，为了减轻这些攻击，需要在 HTTP 头部配上，
...
Promise.resolve(100);
// 直接返回一个 rejected 状态
Promise.reject("some error").then().catch((e)=>{return new Error()}).then();
```
...
**then catch 会继续返回 Promise，如果其中没有报错，都会返回 resolve 状态的 Promise(注意：catch 也是)。
