...
// 默认枚举，不分配的情况，会自动分配 0 1 2 3
console.log(`Rose${Gender.GIRL}`); // 0
console.log(`Jack${Gender.BOY}`); // 1
...
// ButtonHTMLAttributes是react ts给我们内置的ts原生dom类型之一
// ButtonHTMLAttributes<HTMLElement> 原生按钮的类型
// AnchorHTMLAttributes<HTMLElement> 原生a链接的类型
...
function fn(name5: string | number) {
  let r;
  // 这里用as 进行断言从ts语法上看需要用到一个临时变量
  const strTemp = name5 as String;
  if (strTemp.length) {
    r = strTemp.length;
  } else {
    const numberTemp = name5 as Number;
    r = numberTemp.toString().length;
  }
...
用`<>`加载对象的前面(加括号后面可以直接点属性),用以断言,语法上可以免去临时变量.
...
function fn(name5: string | number) {
  ...
  // 这里用as 进行断言从ts语法上看需要用到一个临时变量
  if ((<String>name5).length) {
    // r = name5.length; // 报错, <>断言对下面的无效,不同于as
    r = (<String>name5).length;
  } else {
    r = (<Number>name5).toString().length;
  }
}
...
// 多个泛型  <T, K>申明泛型T,K
// 元组的交换 [boolean,number] = [number,boolean]
...
需求: 约束对象必须带 length 属性

想要约束对象有 length 属性,但是如下约束时,对象只能是数组,局限性太大
...
如下约束对象必须有 length 属性,对象可以是数组,对象,函数等等
...
// new出来的的对象中可能推入任何值，如手动设定成any或者允许自动定义any类型，在运行中,使用方法会产生ts捕捉不到的错误
console.log(queue.pop().toFixed()); // 手动或自动设为any，能消除ts报错，但运行时会产生错误
console.log(queue.pop().length);
```

类的定义与上面完全相同,通过在使用时给泛型定义类型来进行更好的约束
...
常用于库的引入申明，库的申明文件可以去 @types 找 （由DefinitelyTyped组织管理，github可搜到）
microsoft.github.io/TypeSearch/  内可搜索
```typescript
// 申明
declare var jQuery: (selector: string) => any

jQuery('#id') // 申明了之后,就不会报 Unresolved function or method jQuery()
```

一般会建单独文件保存类型，ts解析器会去查找项目中所有的 .d.ts结尾的文件
```typescript
declare var jQuery: (selector: string) => any
```
如果没有加载到，目录下添加tsconfig.json文件
```json
{
  "include": ["**/*"]
}
...
装完高质量申明库，jQuery就有类型了
```shell
yarn add @types/jQuery
```


## Utility Types 官方工具类型
...
// Partial<Todo> 部分属性覆盖功时，覆盖对象可用被覆盖类型加上Partial包装
